...
function normalizeCreatePayload(payload) {
    ...
    const resolvedDesignation = normalizedRole === "EMPLOYEE"
        ? "EMPLOYEE"
...
function serializeApprovalRuleConfig(config) {
    return `${APPROVAL_RULE_CONFIG_PREFIX}${JSON.stringify({
        ...
        minimumApprovalPercentage: Number.isFinite(Number(config?.minimumApprovalPercentage))
...
export async function updateApprovalRuleByAdmin(req, res) {
    try {
        ...
        const requiredApproverIds = Array.from(new Set(rawRequiredApproverIds
        ...
        if (requiredApproverIds.length > 0) {
            ...
            const invalidApprover = approvers.find((approver) => !selectedDesignations.has(String(approver.designation || "").toUpperCase()));
            if (invalidApprover) {
                return res.status(400).json({
                    success: false,
