...
export async function register(req, res) {
    try {
        ...
        try {
            ...
            const countryData = countries.find((c) => c.name?.common?.toLowerCase() === normalizedCountry.toLowerCase());
...
export const forgotPassword = async (req, res) => {
    try {
        ...
        let resetToken;
        try {
            resetToken = jwt.sign({ id: user.id, type: "password-reset" }, process.env.JWT_SECRET, { expiresIn: "15m" });
        }
        catch (jwtError) {
