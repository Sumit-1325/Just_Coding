...
function getApprovalSnapshot(expense) {
    ...
    return {
        ...
        minimumApprovalPercentage: Number.isFinite(minPct) && minPct >= 1 && minPct <= 100 ? Math.round(minPct) : 100,
...
function getApprovalEvaluation(approvalRequests, snapshot) {
    ...
    const threshold = Number.isFinite(Number(snapshot.minimumApprovalPercentage))
        ...
        : 100;
    const requiredPending = approvalRequests.some((item) => requiredApproverIdSet.has(item.approverId) && item.status === "PENDING");
    const requiredRejected = approvalRequests.some((item) => requiredApproverIdSet.has(item.approverId) && item.status === "REJECTED");
    const cfoApproved = approvalRequests.some((item) => item.status === "APPROVED" && String(item.approver?.designation || "").toUpperCase() === "CFO");
...
export async function createExpenseDraftOrSubmit(req, res) {
    try {
        ...
        const { description, expenseDate, category, amount, currencyCode, paidBy, remarks, receiptUrl, submit = false, } = req.body || {};
...
export async function updateDraftExpense(req, res) {
    try {
        ...
        const { description, expenseDate, category, amount, currencyCode, paidBy, remarks, receiptUrl, } = req.body || {};
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (snapshot.approverSequence) {
                ...
                const canBypassSequenceForManagerStep = managerFirstEnabled &&
                    isManagerStep &&
                    !isEmployeeManager &&
